...
This module now handles only Stage 4 (Retrieval). Query expansion, context
compression (reranking), and answer generation have been split into
query_module, reranking_module, and generation_module respectively.
"""
...
from . import query_module, reranking_module, generation_module
...
def filter_subgraph(subgraph, top_relations, max_edges=20):
    ...
    for edge in subgraph["edges"]:
        ...
        if len(edges) >= max_edges:
            break
    return {"nodes": subgraph["nodes"], "edges": edges}
...
def retrieve(config, query, query_result, mode=None):
    """Pure retrieval: vector scoring + KG traversal + context assembly.
    ...
    query : str
    query_result : dict
        Output of query_module.run() containing keywords, retrieval_query, query_vec.
    mode : str | None
        Overrides config.retrieval_mode when set.

    Returns
    -------
    dict with entities, chunks, relations, subgraph, facts, raw_context, references, context_chunks.
    """
    retrieval_mode = mode or config.retrieval_mode
    keywords = query_result["keywords"]
    query_vec = query_result["query_vec"]
    ...
    subgraph = multi_hop_expand(
        graph, seed_nodes, hops=config.subgraph_hops, max_nodes=config.max_subgraph_nodes,
    )
    ...
    subgraph = filter_subgraph(subgraph, top_relations, max_edges=config.max_subgraph_edges)
    ...
    graph_context_chunks = expand_chunks_with_graph_context(
        chunk_scores, chunk_store, related_doc_ids, config.max_context_chunks + 2,
    )
    top_chunks = dedupe_scored_items(top_chunks + graph_context_chunks, config.top_k_chunks)
    selected_kv = select_kv_items(kv_items, subgraph, top_chunks, chunk_store, config.max_context_facts)
    structured_context = build_structured_context(config, subgraph, selected_kv, top_chunks, chunk_store)
    ...
    return {
        ...
        "entities_structured": structured_context["entities"],
        "chunks": top_chunks,
...
def run(config, query, mode=None):
    """Full pipeline: query -> retrieve -> rerank -> generate (backward compat)."""
    query_result = query_module.run(config, query)
    retrieval_result = retrieve(config, query, query_result, mode=mode)
    compressed = reranking_module.run(config, query, retrieval_result["raw_context"])
    answer = generation_module.run(config, query, retrieval_result["raw_context"], compressed)
